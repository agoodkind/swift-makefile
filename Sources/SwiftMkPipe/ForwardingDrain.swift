...
/// sink can block. A forwarding sink that writes synchronously to standard output blocks
/// under downstream backpressure. A blocked sink blocks only its forwarder, not the reader
/// or the caller. `detach()` frees the queued chunks, so a wedged sink retains only the
/// drain object and the one chunk still in its write, which is inherent to a synchronous
/// write that cannot be interrupted and is bounded in practice because the
/// continuous-integration log collector drains standard output and error.
///
public final class ForwardingDrain: @unchecked Sendable {
  private let handle: FileHandle
  private let capturing: Bool
  private let sink: (@Sendable (Data) -> Void)?
  private let onCloseError: (@Sendable (String) -> Void)?
  ...
  ///   - onCloseError: an optional log sink invoked if closing the read handle fails
  ///     during `detach()`; the drain writes nowhere itself so the caller owns formatting.
  @preconcurrency
  public init(
    handle: FileHandle,
    ...
    onCloseError: (@Sendable (String) -> Void)? = nil
  ) {
    self.handle = handle
    self.capturing = capturing
    self.sink = onChunk
    self.sharedGroup = sharedGroup
    self.onCloseError = onCloseError
    if onChunk == nil {
      forwarderQueue = nil
    } else {
  ...
  public func waitUntilFinished(deadline: DispatchTime) -> Bool {
  ...
  public func detach() {
    ...
    // forwarder holding this drain cannot keep the descriptor open. If the join times out,
    // the reader may still be using the descriptor, so leave the close to the owning pipe
    // rather than race the reader.
    if joined {
      closeReadHandle()
    }
    stateLock.lock()
  ...
  /// Close the read end after the reader has stopped. `FileHandle` tracks its own closed
  /// state, so a later close by the owning pipe is a no-op. Returns whether the close
  /// succeeded; a failure is logged through the injected sink and otherwise ignored, since
  /// the owning pipe still closes the descriptor.
  @discardableResult
  private func closeReadHandle() -> Bool {
    do {
      try handle.close()
      return true
    } catch {
      onCloseError?("closing drained read handle failed: \(error)")
      return false
    }
  }
  ...
  private func scheduleForwarderLocked() {
    ...
    forwarding = true
    // Capture self strongly, symmetric with the reader thread: the forwarder owes the group
    // a leave() for the completion it may claim, so it must outlive a caller that dropped its
    // reference and now waits only on the shared group. It runs bounded (until pending drains
    // or the drain is cancelled) and releases self on return, so this is not a cycle.
    queue.async {
