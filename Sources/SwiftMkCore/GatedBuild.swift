...
public enum GatedBuild {
  ...
  public static func run(_ request: Request) -> Int32 {
    ...
    //
    // Unconditional on purpose, including on CI. A caller that owns gating elsewhere
    // (the release job, whose CI already gated the same commit in its own job) routes
    // through `swift-mk build`, whose `GateProof` mark authorizes the compile through
    // live process ancestry. An environment-read skip here would let any caller mint
    // a receipt by setting values, which is exactly what the receipt exists to
    // prevent.
