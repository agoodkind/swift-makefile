  ...
  public static func prune(maxBytes: UInt64, path: String? = nil) throws -> CachePruneResult {
    ...
    if currentBytes > maxBytes {
      for entry in snapshot.entries.sorted(by: pruneEntrySort) {
        ...
        // Isolate per-entry failures: a single undeletable entry, like a
        // permission error or a file vanishing mid-run, must not discard the
        // eviction progress already made.
        do {
