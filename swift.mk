...
SWIFT_MK_MODULES ?=
ifeq ($(strip $(SWIFT_MK_SKIP_FETCH)),1)
SWIFT_MK_FETCHED_MODULES := $(foreach m,$(SWIFT_MK_MODULES),$(if $(wildcard .make/$(m) $(if $(strip $(SWIFT_MK_DEV_DIR)),$(SWIFT_MK_DEV_DIR)/$(m))),,$(error swift-makefile expected .make/$(m)$(if $(strip $(SWIFT_MK_DEV_DIR)), or $(SWIFT_MK_DEV_DIR)/$(m),); rerun without SWIFT_MK_SKIP_FETCH)))
else
