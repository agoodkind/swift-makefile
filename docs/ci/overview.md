...
CI skips the build, test, and lint work on a push or pull request that changes nothing those gates depend on, and every required check still reports green. The decision is engine-owned and automatic, so a consumer sets nothing.
...
The detector runs everything whenever it cannot be certain: an event that is neither a push nor a pull request, a missing or zero or non-ancestor base, an unresolvable merge-base, any git failure, or a build-graph read that fails.
