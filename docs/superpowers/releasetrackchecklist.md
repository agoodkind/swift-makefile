...
3. For a stable release, provide one source selector: `candidate-tag`, or `source-sha` with `allow-source-sha: true`.
4. Confirm that the candidate release is a published GitHub pre-release and contains an artifact matching `candidate-asset-pattern`.
