...
- New Swift files get an Xcode-style header. Run `git config user.name` and `git config user.email`, then stamp `Created by NAME <EMAIL> on YYYY-MM-DD`, substituting the two configured values and keeping the angle brackets around the address. Do not copy a commit `Co-authored-by` identity into a file header. Do not hardcode a person name. GitHub Actions disables the `file_header` rule because hosted runners have no git identity.
