...
extension EnvironmentSerialized {
  @Suite enum LoggingTests {
    ...
    private static func decodeLogRecord(_ line: String) throws -> LoggingTestLogRecord {
      let data = Data(line.utf8)
      return try JSONDecoder().decode(LoggingTestLogRecord.self, from: data)
    }
  }
}

// MARK: - LoggingTestLogRecord

/// One decoded JSONL log line, at file scope so its `CodingKeys` does not push the
/// nested `LoggingTests` suite past the two-level type-nesting limit.
private struct LoggingTestLogRecord: Decodable {
  let traceID: String
  let spanID: String
